...
class MultiAgentBase:
    ...
    def reset(self) -> Dict:
        ...
        # Reset mesh library
        if len(self.world.agents) > 1 and hasattr(self, '_meshlib'):
            self._reset_meshlib()
    ...
    def step(self, actions, dt=1 / 30., MODE=0):
        ...
        for agent in self.world.agents:
            if agent.id in actions.keys():
                action = actions[agent.id]
                agent.step_dynamics(action, dt=dt)#, MODE=MODE)

        # Get agents' sensory measurement
        self._sensor_capture()
        observations = {_a.id: _a.observations for _a in self.world.agents[0:1]}

        # Check terminal conditions
        dones = dict()
        infos_from_terminal_condition = dict()
        terminal_condition = self.config['terminal_condition']
        for agent in self.world.agents[0:1]:
    ...
    def _randomly_place_agent(self, agent: Car):
        ...
        def _close_to_any_agent(_pose, _dist_thresh=30):
            return np.any([
                np.linalg.norm(_pose[:2] - _a.ego_dynamics.numpy()[:2]) <
                _dist_thresh for _a in self.world.agents
            ])

        resample = True
        while resample:
            if False:  # NOTE: check dist_thresh in _close_to_any_agent
                ...
                tgt_idx = np.random.randint(0, road.shape[0])

            tgt_pose = road[tgt_idx].copy()

            resample = _close_to_any_agent(tgt_pose)

        lat_noise = self._rng.choice(
            [-1, 1]) * self._rng.uniform(*self.config['init_lat_noise_range'])
        theta = tgt_pose[2]
        tgt_pose[0] += lat_noise * np.cos(theta)
        tgt_pose[1] += lat_noise * np.sin(theta)
